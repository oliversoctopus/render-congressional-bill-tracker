...
if bill_input:
    try:
        ...
        # Bill header with short title or truncated full title
        if not df.empty:
            # Try to get a short title first
            bill_title = None
            
            # Check if we have a short title in the data
            if 'short_title' in df.columns and df['short_title'].values[0]:
                bill_title = df['short_title'].values[0]
            elif 'title' in df.columns and df['title'].values[0]:
                # Use full title but truncate if needed
                full_title = df['title'].values[0]
                if len(full_title) > 100:
                    bill_title = full_title[:97] + "..."
                else:
                    bill_title = full_title
            
            if bill_title:
                st.subheader(f"Bill Analysis: {bill_type.upper()}.{bill_input} - {bill_title}")
            else:
        ...
        if not actions_df.empty and len(actions_df) > 1:
            st.subheader("📅 Legislative Activity Timeline")
            
            # Create cumulative action count
            actions_df = actions_df.sort_values('date')
            actions_df['cumulative_actions'] = range(1, len(actions_df) + 1)
            
            fig = px.line(actions_df, x='date', y='cumulative_actions',
                         title='Cumulative Legislative Actions',
                         markers=True)
            
            # Add annotations for key events
            key_events = actions_df[
                actions_df['text'].str.contains(
                    'passed|reported|committee|hearing', 
                    case=False, 
                    na=False
                )
            ].head(5)
        ...
        if model_package:
            ...
            # Check if bill has already passed House/Senate (define at the beginning)
            ...
            feature_data = {
                ...
                'title_length': len(df['title'].values[0]) if not df.empty else 100,
                'title_word_count': len(df['title'].values[0].split()) if not df.empty else 20,
            ...
            if has_passed_house or has_passed_senate:
                ...
                # Now that legislative_velocity exists, we can boost it
                if 'legislative_velocity' in feature_data:
                    feature_data['legislative_velocity'] = max(feature_data['legislative_velocity'], 2.0)
