...
def fetch_public_comments(docket_id='CMS-2025-0001'):  # Example docket for a bill/regulation
    url = f'https://api.regulations.gov/v4/comments?filter[docketId]={docket_id}&api_key=DEMO_KEY'  # Use DEMO_KEY for free access
    response = requests.get(url)
    if response.status_code == 200:
        data = response.json()
